...
const AddCourse = () => {
  ...
  const editRef = useRef(null); // ✅ Correct reference name
  ...
  const addLecture=()=>{
    ...
    chapters.map((chapter)=>{
      if(chapter.chapterId === currentChapterId){
        const newLecture={
          ...lectureDetails,
          lectureOrder:chapter.chapterContent.length > 0 ? chapter.chapterContent.slice(-1)[0].lecutreOrder +1:1,
          lectureId: uniqid()
        };
    ...
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div ref={editRef}></div> {/* ✅ Use editRef instead of editorRef */}
        </div>
        ...
        <div className="flex flex-col gap-1">
        <p>Discount</p>
        <input onChange={e=>setDiscount(e.target.value)} value={discount} type="number" placeholder="0" min={100} className="outline-none md:py-2.5 w-28 px-3 rounded border border-gray-500" required/> 
        </div>
        <div>
              ...
              <div className="p-4">
                {chapter.chapterContent.map((lecture,lectureIndex)=>(
                  <div key={lectureIndex} className="flex justify-between items-center mb-2">
                    <span>{lectureIndex+1}{lecture.lectureTitle}-{lecture.lectureDuration}mins-<a href={lecture.lectureUrl}target="_blank" className="text-blue-500">LLink</a>-{lecture.isPreviewFree ? 'Free Preview':'Paid'}</span>
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <button type="button" className="w-full bg-blue-400 text-white px-4 p-2 rounded" onClick={addLecture}>Add</button>
                <img onClick={()=>setShowPopup(false)} src={assets.cross_icon} className="absolute top-4 right-4 w-4 cursor-pointer"alt=""/>
              </div>
              </div>
          )}
        </div>
        <button type='submit' className="bg-black text-white w-max py-2.5 px-8 roundex my-4">
          ADD
        </button>
